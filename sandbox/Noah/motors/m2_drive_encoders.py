...
def main():
    ...
    while time_s != 0:
        ...
        degrees_to_target = int(input("Enter a distance to drive (inches): ")) * 90
        if degrees_to_target == 0:
            break

        left_motor.run_to_rel_pos(degrees_to_target, spe, ev3.Motor.STOP_ACTION_BRAKE)
        right_motor.run_to_rel_pos(degrees_to_target, spe, ev3.Motor.STOP_ACTION_BRAKE)
