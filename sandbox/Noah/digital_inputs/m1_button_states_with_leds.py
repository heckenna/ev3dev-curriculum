...
Much like we did in Junheng of the motors unit, later we will show you different (better) ways to use buttons.
...
def main():
    ...
    while True:
        if btn.up:
            ...
            print("down")
            ev3.Leds.set_color(ev3.Leds.LEFT, colors[var])
            ev3.Leds.set_color(ev3.Leds.RIGHT, colors[var])
            var += 1
            if var >= len(colors):
                var = 0
            while btn.down:
